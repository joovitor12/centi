...
def upgrade() -> None:
    ...
    if not inspector.has_table("recurring_appointments"):
        op.create_table(
    ...
    index_name = op.f("ix_recurring_appointments_id")
    indexes = {idx["name"] for idx in inspector.get_indexes("recurring_appointments")}
...
def downgrade() -> None:
    ...
    op.execute(
        f"DROP INDEX IF EXISTS {op.f('ix_recurring_appointments_id')}"
    )
    op.execute("DROP TABLE IF EXISTS recurring_appointments")
